...
this.router.get('/students/:id', this.GpGetNounById);
    }
...
public GpGetNounById(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into studentsController.ts: GpGetNounById');
        new ApiAdaptar().get(Constant.SCHOOLURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
            new CustomLogger().showLogger('info', 'Exit from studentsController.ts: GpGetNounById');
        }).catch(err => {
            res.send(err);
        });
    }

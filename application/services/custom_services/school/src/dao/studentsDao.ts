...
public GpGetNounById(studentsId, callback){
new CustomLogger().showLogger('info', 'Enter into studentsDao.ts: GpGetNounById')

this.students.findById(studentsId).then((result)	=>	{
new CustomLogger().showLogger('info', 'Exit from studentsDao.ts: GpGetNounById');
callback(result);
}).catch((error)=>{
callback(error);
});}
